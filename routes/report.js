...
// ------ Report Function Dr -----------
async function reportDr(client, up) {
   var path,form,s;
   var res = {};
   var context;
   res.username = client.credentials.username;
...
//##################################################
  // (2) -------------- report to selected ---------------
  // انه غير مناسب
  await sleep(Math.floor( Math.random() * 100) + 5 );
  s = await tag("ig_its_inappropriate_v1");
  if (s == null) {
       consloe.log("report error 2s");
       //log("report error 3");
...
//##################################################
  //----step (5) -------------
  //الابلاغ عن مبيعات ممنوعة
  await sleep(Math.floor( Math.random() * 100) + 5 );
  s = await tag("ig_sale_of_illegal_or_regulated_goods_v3");
  if (s === null) {
    console.log("error 5s");
    resolve({err: "report error 5s", username:res.username});
    return;
...
module.exports = {reportSpam, reportSol, reportScam, reportDr}
